 ...
 * updated by writer :
 * update :
 * description : 회원 업로드 파일 저장에 사용
 */
@Component
@Slf4j
public class FileStoreOfSelfPicture {

    @Value("${file.selfPicture.dir}")
    private String fileDir; // 파일 경로
    @Value("${directory.selfPicture.dir}")
     ...
     */
    public List<SelfPicture> storeFiles(List<MultipartFile> multipartFiles) throws IOException {
        // 파일 담을 list 생성
        List<SelfPicture> storeFileResult = new ArrayList<>();
     ...
     */
    public SelfPicture storeFile(MultipartFile multipartFile) throws IOException {
        ...
        return SelfPicture.createSelfPicture(originalFileName, storeFileName);
    }
